# 根据coolmath-games-data.js生成HTML文件

# 读取coolmath-games-data.js文件内容
$coolmathDataContent = Get-Content -Path "f:\DATA\MyWorkspace\h5game\traffic games\js\coolmath\coolmath-games-data.js" -Raw

# 提取游戏数据
# 简单地提取数组内容，去掉开头和结尾的部分
$gamesJson = $coolmathDataContent -replace '^.*?var coolmathGamesData = \[', ''
$gamesJson = $gamesJson -replace '\];.*$', ''

if ($gamesJson) {
    
    # 修复JSON格式（添加引号等）
    $gamesJson = $gamesJson -replace '"(name|imageUrl|firameUrl|link|description|keywords)"\s*:', '"$1":'
    $gamesJson = $gamesJson -replace '\s*,\s*$', ''
    
    # 尝试解析JSON
    try {
        $games = ConvertFrom-Json "[$gamesJson]"
        
        # 遍历每个游戏
        foreach ($game in $games) {
            $gameName = $game.name
            $imageUrl = $game.imageUrl
            $iframeUrl = $game.firameUrl
            $link = $game.link
            $description = $game.description
            $keywords = $game.keywords
            
            # 生成HTML文件名
            $htmlFileName = $link -replace 'coolmath/', ''
            $htmlFilePath = "f:\DATA\MyWorkspace\h5game\traffic games\coolmath\$htmlFileName"
...
<head>
    ...
    <title>$gameName - Play Free Online Game</title>
...
<body>
    ...
    <script src="../js/data/two-player-games-data.js"></script>
...
"@
            ...
            Write-Host "生成了文件: $htmlFileName"
        }
    } catch {
        Write-Host "解析JSON失败: $_"
    }
} else {
    Write-Host "未能提取游戏数据"
}
