...
// 读取driving-games-data.js文件
const drivingGamesDataPath = path.join(__dirname, 'js', 'data', 'driving-games-data.js');
const drivingGamesDataContent = fs.readFileSync(drivingGamesDataPath, 'utf8');

// 提取游戏数据
const gamesDataMatch = drivingGamesDataContent.match(/var drivingGamesData = \[(.*?)\];/s);
if (!gamesDataMatch) {
...
// 确保driving目录存在
const drivingDir = path.join(__dirname, 'driving');
if (!fs.existsSync(drivingDir)) {
...
const generatePage = (game) => {
    ...
    const filePath = path.join(drivingDir, `${safeFileName}.html`);
...
<body>
            ...
            <div class="detail-container">
                <div class="detail-content">
                    <div class="detail-block">
                        <h2 class="detail-title">${gameName} - Master the Art of Driving</h2>
                        <p class="detail-description" id="objectiveText">Play ${gameName}, an exciting driving game that challenges you to master the art of precision driving. Navigate through challenging environments and complete various driving tasks.</p>
                        <p>${gameName} offers a thrilling driving experience with its realistic controls and challenging scenarios. Perfect for fans of driving simulation games, this game combines skill, precision, and excitement in every mission.</p>
                    </div>

                    <div class="detail-block">
                        <h2 class="detail-title">How to Play ${gameName}</h2>
                        <p>In ${gameName}, your goal is to complete various driving tasks with precision and skill. Use the arrow keys or WASD to control your vehicle, and use the spacebar for braking. Navigate through challenging environments, avoid obstacles, and complete missions to progress through the game.</p>
                        <p>As you progress, you'll unlock new vehicles and missions with increasing difficulty. The game features realistic physics and responsive controls, allowing you to navigate tight spaces and challenging terrain. Pay attention to the environment and plan your moves carefully to succeed in each mission.</p>
                    </div>

                    <div class="detail-block">
                        <h2 class="detail-title">Features of ${gameName}</h2>
                        <p>${gameName} offers a range of features that make it one of the most exciting driving games available online. These include:</p>
                        <ul>
                            <li>Realistic driving physics and controls</li>
                            <li>Multiple challenging missions to complete</li>
                            <li>A variety of vehicles to drive</li>
                            <li>Detailed environments and scenarios</li>
                            <li>Progressive difficulty levels</li>
                        </ul>
                        <p>Whether you're a seasoned driving game veteran or a casual player looking for some driving fun, ${gameName} offers an engaging driving experience that will keep you coming back for more. So get behind the wheel and start driving!</p>
                    </div>
    ...
    <script src="../js/driving/${safeFileName}-data.js"></script>
